      ...
      <div className="space-y-4">
        {results.map((res, idx) => (
          <div 
            ...
            className="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm hover:shadow-md hover:border-slate-200 hover:-translate-y-0.5 active-spring transition-all duration-300 cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-4 animate-fade-in-up"
            style={{ animationDelay: `${idx * 75}ms` }}
          >
            <div>
              <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-md uppercase tracking-wider">{res.subject}</span>
              <h3 className="font-bold text-slate-900 text-base mt-3">{res.title}</h3>
              <p className="text-xs font-semibold text-slate-450 mt-1.5">Submitted on {res.date}</p>
            </div>
            ...
            <div className="flex items-center space-x-6 self-end md:self-auto shrink-0">
              ...
              <button className="p-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-xl transition-all active-spring">
