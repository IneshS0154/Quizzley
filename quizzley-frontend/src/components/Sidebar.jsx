...
export default function Sidebar({ currentPage, setCurrentPage, onLogout, role }) {
  ...
  const menuItems = role === 'ADMIN' ? adminMenuItems : (role === 'QUIZ_MANAGER' ? quizManagerMenuItems : studentMenuItems);
  ...
  const bottomItems = (role === 'ADMIN' || role === 'QUIZ_MANAGER') ? adminBottomItems : studentBottomItems;
  ...
  const handleItemClick = (id) => {
    ...
    if (id === 'notifications' && role !== 'ADMIN' && role !== 'QUIZ_MANAGER') {
      alert("Notifications: You have no new notifications.");
      return;
    }
    if (id === 'settings' && role !== 'ADMIN' && role !== 'QUIZ_MANAGER') {
      alert("Settings: Admin system settings configuration panel.");
      return;
    }
    setCurrentPage(id);
  };
      ...
      <div className="p-6">
        ...
        <div className="px-3 mb-4">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            {role === 'ADMIN' ? 'System Admin' : (role === 'QUIZ_MANAGER' ? 'Quiz Manager Portal' : 'Student Portal')}
          </span>
        </div>
